...
def normalize_points(points):
    ...
    # scale = (np.sqrt(2) / np.mean(np.linalg.norm(points - mean[0], axis=1))) 
    # scale = np.mean(np.linalg.norm(points - mean, axis=1))
    
    # Create the normalization matrix
    T = np.array([[sx, 0, -sx * mean[0]],
                  [0, sy, -sy * mean[1]],
                  [0, 0, 1]])
...
def norm_eight_point_algo(points1, points2):
    ...
    F = eight_point_algo(norm_points1, norm_points2)
    F = T2.T @ F @ T1
...
def plot_epipolar_lines(F, img1, img2, points1, points2):
    ...
    for i in range(len(points1)):
        ...
        distance = abs(a2 * point1[0] + b2 * point1[1] + c2) / np.sqrt(a2**2 + b2**2)
...
if __name__ == '__main__':
    ...
    points1 = readFile(file_path = './assets/pt_2D_1.txt')
    points2 = readFile(file_path = './assets/pt_2D_2.txt')
    ...
    # average_distance1_norm, average_distance2_norm = plot_epipolar_lines(F_norm/ F_norm[2,2], img1, img2, points1, points2)
